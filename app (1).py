...
# --- FUNÇÃO DA FONTE CORRIGIDA PARA A NUVEM ---
def carregar_fonte_segura():
    font_filename = "Montserrat-Bold.ttf"
    tamanho_fonte = 55
    
    # 1. TENTA LER O ARQUIVO LOCAL (A melhor solução)
    if os.path.exists(font_filename):
        try:
            return ImageFont.truetype(font_filename, tamanho_fonte)
        except:
            pass
            
    # 2. SE NÃO TIVER, TENTA BAIXAR (Pode ser bloqueado pelo servidor na nuvem)
    ...
    # 3. FALLBACK PARA FONTES DA NUVEM (Streamlit Cloud usa Linux Debian)
    fontes_fallback = [
    ...
    # 4. ÚLTIMO RECURSO (É o que gera a letra miúda, queremos evitar chegar aqui)
    ...
    def draw_txt(text, y):
        ...
        try: 
            ...
            try:
                ...
                w = len(text) * 10 # Estimativa tosca

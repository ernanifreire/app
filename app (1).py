...
# --- FUNÇÃO DA FONTE CORRIGIDA (LÊ O ARQUIVO LOCAL) ---
def carregar_fonte_segura():
    tamanho_fonte = 55
    
    # Descobre a pasta exata onde o site está rodando no servidor
    diretorio_atual = os.path.dirname(os.path.abspath(__file__))
    caminho_fonte = os.path.join(diretorio_atual, "Montserrat-Bold.ttf")
    
    # 1. TENTA LER O ARQUIVO FÍSICO DA PASTA
    if os.path.exists(caminho_fonte):
        try:
            return ImageFont.truetype(caminho_fonte, tamanho_fonte)
        except Exception as e:
            st.error(f"Fonte encontrada, mas deu erro ao carregar: {e}")
    else:
        # Se não achar o arquivo, avisa o usuário na tela
        st.warning("⚠️ Arquivo 'Montserrat-Bold.ttf' não encontrado na pasta do projeto! Faça o upload no GitHub.")
            
    # Último recurso (Gera a letra minúscula caso a fonte não tenha sido upada)

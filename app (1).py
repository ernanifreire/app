...
# --- FUNÇÃO DA FONTE 100% BLINDADA PARA NUVEM ---
def carregar_fonte_segura():
    tamanho_fonte = 55
    
    # 1. TENTA LER O ARQUIVO LOCAL (Se você subiu o .ttf no GitHub, ele acha aqui)
    if os.path.exists("Montserrat-Bold.ttf"):
        try:
            return ImageFont.truetype("Montserrat-Bold.ttf", tamanho_fonte)
        except:
            pass
            
    # 2. SE NÃO TIVER NO GITHUB, BAIXA DIRETO PARA A MEMÓRIA RAM (Sem salvar no disco)
    urls = [
    ...
    for url in urls:
        try:
            ...
            # Verifica se o arquivo é válido (maior que 20kb)
            if response.status_code == 200 and len(response.content) > 20000:
                # Transforma o download direto em um arquivo na memória RAM
                font_bytes = io.BytesIO(response.content)
                return ImageFont.truetype(font_bytes, tamanho_fonte)
        except: 
            continue
            
    # 3. FALLBACK PARA FONTES DA NUVEM (Se o download falhar, tenta fontes de sistema em tamanho grande)
    fontes_fallback = [
        "Arial.ttf", 
        "arial.ttf",
        "DejaVuSans-Bold.ttf",
    ...
    for f in fontes_fallback:
        try: 
            return ImageFont.truetype(f, tamanho_fonte)
        except: 
            continue
            
    # 4. ÚLTIMO RECURSO (É o que gera a letra miúda)
    ...
    def draw_txt(text, y):
        try: 
            ...
            try:
                ...
                w = len(text) * 10 # Fallback caso a fonte não tenha a função de tamanho

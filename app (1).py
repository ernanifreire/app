...
# --- NOVA FUNÇÃO AUXILIAR PARA BAIXAR A FONTE ---
def carregar_fonte_segura():
    font_filename = "Montserrat-Bold.ttf"
    # Link direto do Google Fonts
    font_url = "https://github.com/google/fonts/raw/main/ofl/montserrat/Montserrat-Bold.ttf"
    
    # Se o arquivo não existe, baixa ele agora
    if not os.path.exists(font_filename):
        try:
            urllib.request.urlretrieve(font_url, font_filename)
        except Exception as e:
            # Se der erro (ex: sem internet), usa a padrão mesmo
            return ImageFont.load_default()
            
    # Retorna a fonte carregada com tamanho 70
    return ImageFont.truetype(font_filename, 70)

# --- FUNÇÃO ATUALIZADA DO BANNER VERTICAL ---
def gerar_vertical(logo_img, icon_pil):
    ...
    # --- AQUI ESTÁ A CORREÇÃO DA FONTE ---
    font = carregar_fonte_segura()
    # -------------------------------------

    def draw_txt(text, y):
        # O método textbbox funciona diferente dependendo da versão do Pillow
        # Vamos usar um try/except para garantir compatibilidade
        try:

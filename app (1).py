...
import base64
import os
import requests  # Usaremos requests que é mais robusto que urllib

# Permite carregar imagens que possam ter pequenos erros de dados
...
st.markdown("Faça upload das logos e baixe tudo pronto.")

# ==============================================================================
# ÁREA DO ÍCONE (COLE SEU CÓDIGO AQUI!)
# ==============================================================================
ICONE_PADRAO_BASE64 = """
...
def carregar_fonte_segura():
    ...
    if not os.path.exists(font_filename):
        try:
            ...
            print(f"Aviso: Não consegui baixar a fonte Montserrat. Erro: {e}")
    ...
    # 2. TENTATIVA (PLANO B): Usar fontes do sistema (Windows ou Linux)
    # Isso garante que o "ç" funcione mesmo sem internet
    fontes_sistema = [
        "Arial.ttf",             # Padrão Windows
        "arial.ttf",             # Padrão Windows (caixa baixa)
        "DejaVuSans-Bold.ttf",   # Padrão Linux (Streamlit Cloud)
        "Roboto-Bold.ttf",       # Comum em Android/Linux
        "LiberationSans-Bold.ttf" # Comum em Linux
    ]
    ...
    # 3. TENTATIVA (ULTIMO CASO): Fonte Padrão
    # Se chegar aqui, o texto vai ficar pequeno, mas é muito raro chegar aqui
    print("Aviso crítico: Nenhuma fonte TTF encontrada. Usando padrão.")
...
def gerar_vertical(logo_img, icon_pil):
    ...
    # CARREGA A FONTE (COM PLANO A, B e C)
...
# --- AQUI ESTÃO OS AJUSTES ---
    ...
    # 1. TEXTO MAIS PRA CIMA
    # Antes era + 180. Mudei para + 140 (sobe 40 pixels)
    draw_txt("Toque na tela", footer_y + 140)
    
    # Antes era + 270. Mudei para + 210 (sobe e aproxima da linha de cima)
    draw_txt("e faça seu pedido!", footer_y + 210)
    
    # 2. LINHA (TRAÇO)
    # Posição Y (Altura): Antes era + 380. Mudei para + 300 (sobe bastante)
    # Comprimento: Antes era 200. Mudei para 130 (fica mais curta)
...
if uploaded_logos:
    ...
    if icon_pil is None: st.warning("⚠️ Ícone não carregado. Cole o código Base64 no script para corrigir.")

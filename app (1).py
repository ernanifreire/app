...
st.sidebar.caption("Se marcado, esta cor será usada em TODOS os banners da lista, ignorando a cor da logo.")
...
def calcular_cor_texto(rgb):
    # Calcula luminância para decidir se texto é preto ou branco
    luminancia = (rgb[0]*299 + rgb[1]*587 + rgb[2]*114)/1000
    if luminancia > 160:
        return "#000000" # Fundo claro -> Texto preto
    else:
        return "#fff8ef" # Fundo escuro -> Texto claro
...
def determinar_cores_finais(logo_img, usar_fundo_branco, usar_manual, cor_manual):
    # Se o usuário escolheu manual, usamos direto
    if usar_manual:
    ...
    if st.button("🚀 Gerar"):
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zf:
            for i, logo_file in enumerate(uploaded_logos):
                ...
                # 1. Determina as cores (Auto ou Manual)
                ...
                # 4. Mostra o Preview (com as cores corretas)
                if i == 0:
